...
//Muqobil Dasturlar To'plami (c) hijriy 1436 (2014 yil)
...
int timeOut=0, navID;
std::string tmp, str2num, serr, gTimeStr;
...
static gpointer thread_func2(gpointer data)
{
    ...
    while(gStarted)
    {
        ...
        if( (timeOut > 0) && (timeOut % 60 == 0) )
        {
            ...
            timeOut = 0;
            cout << gTimeStr << " " << r << " " << amv_median[r] << endl;
                    ...
                    if (gTimeStr.compare("0800") == 0)
                    {
                        ...
                        double kW = median(awatt_per_hour, awatt_per_hour.size());
    ...
    while ( actual  >= 0 )
            {
                ...
                if(c  == len)
                {
                    ...
                    //gtk_text_buffer_insert_with_tags_by_name(buffer, &txt_iter, &str2num[0], -1, "big", "red_foreground" ,"wide_margins", NULL);
                    tmp.clear();
                }
...
cb_timeout( gpointer data )
{
    ...
    if(gStarted){
        ...
        if(corr_order) {
                ...
                if ( std::stof(str2num, nullptr) < gLow ||  std::stof(str2num, nullptr) > gUp ) gst_element_set_state (GST_ELEMENT(playbin2), GST_STATE_PLAYING);
        ...
        gtk_text_buffer_insert_with_tags_by_name(buffer, &txt_iter, &str2num[0], str2num.length(), "big", "red_foreground" , NULL);
...
static void start_usb_accetp(GtkWidget *win, gpointer data)
{
    ...
    gtk_text_buffer_set_text (buffer, "", 0);
    gtk_text_buffer_get_iter_at_offset (buffer, &txt_iter, 0);
    gtk_text_buffer_insert_with_tags_by_name(buffer, &txt_iter, &str[0], -1, "right_justify", "red_foreground" ,"wide_margins", NULL);*/
  ...
  if(hdev==NULL)
            {
                ...
                //goto fin;
                gtk_text_buffer_insert_with_tags_by_name(buffer, &txt_iter, &str[0], -1, "big", "red_foreground" ,"wide_margins", NULL);
                return;
            }
         else
...
/*    pipeline = gst_pipeline_new ("audio-player");
  ...
  gtk_text_buffer_create_tag (buffer, "left", "justification", GTK_JUSTIFY_LEFT, NULL);
    ...
    gtk_text_buffer_create_tag (buffer, "red_foreground", "foreground", "blue", NULL);
